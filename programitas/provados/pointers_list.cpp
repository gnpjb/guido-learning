...
numberslist *initialize(int number)
{
    numberslist *result = new numberslist;
    result->number = number;
    result->p_next = NULL;
    return result;
}

void grow1(numberslist *list,const int &number)
{
        if((*list).p_next==NULL)
        {
            list->p_next = initialize(number);
        }
        else
        {
...
{
    numberslist *t_pointer = list->p_next;
    list->p_next = initialize(*number);
